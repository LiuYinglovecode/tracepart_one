...
public class BinaryJedis implements BasicCommands, BinaryJedisCommands, MultiKeyBinaryCommands,
    AdvancedBinaryJedisCommands, BinaryScriptingCommands, BinaryTSDBCommands, Closeable {
  ...
  /**
   * bde all the received requests in real time.
   * <p>
  ...
  public byte[] bde(final byte[] key) {
    checkIsInMultiOrPipeline();
    client.bde(key);
    return client.getBinaryBulkReply();
  }
  ...
  public Long hstrlen(byte[] key, byte[] field) {
	...
	@Override
	public Long tadd(byte[] key, Map<byte[], byte[]> tvPair) {
		checkIsInMultiOrPipeline();
	    client.tadd(key, tvPair);
	    return client.getIntegerReply();
	}
}
