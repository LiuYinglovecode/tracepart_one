...
public class CngoldDownload {
    ...
    public static void main(String[] args) {
        ...
        cngoldDownload.newsInfo("http://gold.cngold.com.cn/20191115d1715n340665414.html");
    }
    //新闻信息
    public void newsInfo(String url) {
        ...
        try {
            String html = HttpUtil.httpGetwithJudgeWord(url, "cngold");
            Thread.sleep(SleepUtils.sleepMin());

            if (html != null) {
                Document document = Jsoup.parse(html);
                String title = document.select("div.new_tit > h1").text().trim();
                newsInfo.put("title", title);//标题
                Elements select = document.select("div.new_tit > div > span");
                for (Element element : select) {
                    if (element.text().contains("来源：")){
                        newsInfo.put("source", element.text().split("来源： ")[1].trim());//来源
                ...
                Elements text = document.select("div.zhaiyaos,div.xw_cont");
                newsInfo.put("text", text.text().trim());//新闻内容
                newsInfo.put("html", text.html());//新闻内容
                String newsId = NewsMd5.newsMd5(text.text().trim());
                newsInfo.put("newsId", newsId);
                Elements img = document.select("center > img");
                if (img.size() != 0) {
