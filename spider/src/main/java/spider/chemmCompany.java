...
public class chemmCompany {
    ...
    private void category(String url) {
        try {
            ...
            for (Element e : select){
                ...
                String s = "http://www.chemm.cn" + href;
                Thread.sleep(3000);
                company(s);
            }
        } catch (Exception e) {
    ...
    private void company(String url){
        try {
            ...
            Elements elements = parse.select("div.ProListMain > ul > li.ProListMainTitle > span > a");
            for (Element element :elements){
                String href = element.attr("href");
                Thread.sleep(3000);
                info(href);
            }
            Elements select = parse.select("#SearchLeft > div.PageBar > div.PageButtons > a");
            for (Element e : select) {
                if ("下一页".equals(e.text())) {
                    String href = "http://www.chemm.cn"+e.attr("href");
        ...
        try {
            ...
            for (Element e : elements){
                ...
                if ("联系方式".equals(e.text())){
                    ...
                    companyInfo.put("website_name",("中国化工机械网"+MD5Util.getMD5String(parse.select("#MemberTitle").text())));
    ...
    private void insert(JSONObject companyInfo) {
        ...
        if (TxtUpdateToMySQL.chemmUpdate(Map)) {
