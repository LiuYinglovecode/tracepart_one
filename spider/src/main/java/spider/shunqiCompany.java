        ...
        try {
            ...
            for (Element e : categoryList) {
                if ("按城市浏览全国公司黄页".equals(e.select(".boxtitle").text())) {
                    for (int i = 2; i < e.select(".boxcontent .listtxt dd a").size(); i++) {
...
//                    }
    ...
    private void detail(String url) {
        ...
        try {
            ...
            companyInfo.put("id", MD5Util.getMD5String(document.select("#logoco h1 span").text().trim()));
    ...
    private void insert(JSONObject companyInfo) {
        ...
        if (TxtUpdateToMySQL.shunqiUpdate(Map)) {
