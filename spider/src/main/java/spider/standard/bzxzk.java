 ...
 */
public class bzxzk {
    private final static Logger LOGGER = LoggerFactory.getLogger(bzxzk.class);
...
//    private static String zookeeper = "172.20.4.213:2181";
    ...
    static {
        mainWebsite = "http://www.bzxzk.net/";
        downMainURL = "http://www.bzxzk.net/e/DownSys/";
        header = new HashMap();
    ...
    public static void main(String[] args) {
        ...
        bzxzk bzxzk = new bzxzk();
        bzxzk.shouye(mainWebsite);
    ...
    private void shouye(String mainWebsite) {
        try {
            ...
            String[] industryStandard = {"gjbz", "hybz", "qtbz"};
            for (String e : industryStandard) {

                if(e == "gjbz") {
                    standard_1(mainWebsite, e, "国内标准", "GB国家标准");
                }else if(e == "hybz"){
                    standard_2(mainWebsite, e, "行业标准");  // url已经是二级各行业标准的细分类页，即显示的三类分类的标题
                }else{
                    standard_1(mainWebsite, e, "其他标准", "其它标准");
                }
            }
//            }
        } catch (Exception e) {
    ...
    private void standard_1(String mainWebsite, String url_end, String category1, String category2) {
        ...
        try {
            ...
            if (null != document) {
                // 获取当前页面链接
                Elements standardList = document.select(".news_list a");
                        ...
                        for (Element i : pageList) {
                            if ("下一页".equals(i.text().trim())) {
                                if (!endPage.equals(i.attr("href").split("_")[1].split("\\.")[0])) {
                                    ...
                                    standard_1(mainWebsite, i.attr("href"), category1, category2);
                                }
                            }
                        }
                    }
    ...
    private void detail(String url, String name, String category1, String category2) {
        ...
        try {
            ...
            if (null != document) {
                ...
                Element div = document.select(".contentdown").last();  // class值为contentdown的元素标签
                String downloadUrl;
                String a_onclick = div.select("a").attr("onclick");
