 ...
 */
public class dzbzw {
    private final static Logger LOGGER = LoggerFactory.getLogger(dzbzw.class);
...
//    private static String zookeeper = "172.20.4.213:2181";
    ...
    static {
        mainWebsite = "http://www.dzbzw.com/down/";
        downMainURL = "http://www.dzbzw.com/e/";
        header = new HashMap();
    ...
    public static void main(String[] args) {
        ...
        dzbzw dzbzw = new dzbzw();
        dzbzw.shouye(mainWebsite);
    ...
    private void shouye(String mainWebsite) {
        try {
            ...
            String[] industryStandard = {"gjbz", "hybz", "gwbz", "qtbz"};
            for (String e : industryStandard) {

                if(e == "gjbz") {
                    standard_1(mainWebsite, e, "国内标准", "GB国家标准");
                }else if(e == "hybz"){
                    standard_1(mainWebsite, e, "行业标准", "行业标准");  // url已经是二级各行业标准的细分类页，即显示的三类分类的标题
                }else if(e == "gwbz"){
                    standard_1(mainWebsite, e, "国际标准", "国际标准");  // url已经是二级各行业标准的细分类页，即显示的三类分类的标题
                }else if(e == "qtbz"){
...
//            }
    ...
    private void standard_1(String mainWebsite, String url_end, String category1, String category2) {
        ...
        try {
            ...
            if (null != document) {
                // 获取当前页面链接
                Elements standardList = document.select(".news_list .list_title a");
                ...
                // 翻页操作
                Elements pageList = document.select(".pages a");  // ".pagecss a"表示class值为pagecss的元素下子元素为a标签
                int endPage;
    ...
    private void detail(String url, String name, String category1, String category2) {
        ...
        try {
            ...
            if (null != document) {
                ...
                Element a = document.select(".contentdown a").last();  // class值为contentdown的元素标签下的最后一个a元素
                String a_onclick = a.attr("onclick");
                String downloadUrl = a_onclick.split("\'")[1];
    ...
    private void download(String url, String name, String category1, String category2, String codeName) {
        ...
        try {
            ...
            info.put("crawlerId", "43");  // 爬虫代码
            Document document = Jsoup.parse(new URL(url).openStream(), "GBK", url);
            String regx = "\\.\\./\\.\\./";
            if (null != document) {
                String downloadUrl2 = document.select("a").first().attr("href").trim().split(regx)[1];
