...
//            }
    ...
    private void detail(String url) {
        ...
        try {
            String html = httpGetWithProxy(url, "公司首页");
            Document document = Jsoup.parse(html);
            companyInfo.put("name", document.select(".com-name").text());
            companyInfo.put("id", MD5Util.getMD5String(document.select(".com-name").text()));
            companyInfo.put("detail", document.select(".r-content tbody").text());
            Elements topInfo = document.select(".data .con-txt li");
            for (Element e : topInfo) {
                String key = e.text().split("：", 2)[0];
                switch (key) {
                    case "所在地":
                        companyInfo.put("location", e.text().split("：", 2)[1]);
                        break;
                    case "企业类型":
                        companyInfo.put("type", e.text().split("：", 2)[1]);
                        break;
                    case "成立时间":
                        companyInfo.put("establishment", e.text().split("：", 2)[1]);
                        break;
                    case "主营行业":
                        companyInfo.put("main_industry", e.text().split("：", 2)[1]);
                        break;
                    case "主营产品":
    ...
    private void insert(JSONObject companyInfo) {
        Map = (Map) companyInfo;
        if (TxtUpdateToMySQL.huangye88Update(Map)) {
