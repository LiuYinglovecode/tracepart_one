...
public class Client extends BinaryClient implements Commands {
  ...
  public void bde(final String key) {
    bde(SafeEncoder.encode(key));
  }
  ...
  @Override
  public void tadd(String key, Map<Long, Double> tvPair) {
	  final Map<byte[], byte[]> bPair = new HashMap<byte[], byte[]>(tvPair.size());
	  for (final Entry<Long, Double> entry : tvPair.entrySet()) {
		  bPair.put(toByteArray(entry.getKey().longValue()), toByteArray(entry.getValue().doubleValue()));
	  }
	  
	  tadd(SafeEncoder.encode(key), bPair);
  }
